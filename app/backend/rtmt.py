...
from agents.tools.general import General_Tools
from agents.tools.webscraper import Web_Tools
...
class RTMiddleTier:  
            ...
            with open(profile_path, "r") as file:  
                try:  
                    ...
                    self.kernels[agent_name].add_plugin(
                        plugin=General_Tools(),
                        plugin_name="general_tools",
                        description="shared tools for all agents"
                )
                    if agent_name == "general_questions_agent":
                        self.kernels[agent_name].add_plugin(  
                            plugin=Web_Tools(),  
                            plugin_name="webscraper",  
                            description="webscraper tools for general questions agent"  
                        )
    ...
    async def _detect_intent_change(self, session: dict):  
        ...
        if intent in self.agent_names and intent != session["current_agent"].get("name"):  
            ...
            summary = await self.summarize_conversation(session["history"])
